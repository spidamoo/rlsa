...
IImage* CD3D9Driver::createScreenShot(video::ECOLOR_FORMAT format, video::E_RENDER_TARGET target)
{
	...
	RECT clientRect;
	{
		...
		clientRect.left = core::max_((long)clientRect.left, 0l);
		clientRect.top = core::max_((long)clientRect.top, 0l);
		clientRect.right = core::min_((long)clientRect.right, (long)displayMode.Width);
		clientRect.bottom = core::min_((long)clientRect.bottom, (long)displayMode.Height );
	}
